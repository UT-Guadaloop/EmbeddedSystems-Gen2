...
AppHibernateEnter(void)
{
    ...
    UARTprintf("To wake, wait for 5 seconds or press WAKE or"
               " RESET\n");
    UARTprintf("See README.md for additional wake sources.\n");
...
main(void)
{
    ...
    // wake source. It is available on MSP_EXP432E401Y BoosterPack 2 (X7-17)
    // and on the breadboard breakout connector (X11-63).  Short to ground to
    // generate a wake request. REMOVE/CHANGE THIS?
    //
    ...
    //
    SysTickPeriodSet(ui32SysClock / 30);
    SysTickEnable();
    SysTickIntEnable();
    IntMasterEnable();
    ...
    UARTprintf("%s\n", g_pcWakeBuf);
    UARTprintf("Welcome to the MSP432E401Y LaunchPad!\n");
