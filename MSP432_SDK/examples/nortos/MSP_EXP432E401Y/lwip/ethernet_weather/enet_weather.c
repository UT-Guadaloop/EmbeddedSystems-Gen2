...
//
//*****************************************************************************
//##### TEST_INSTRUCTIONS BEGIN #####
//
// If no Network Connection
//
// App should display the first city and hault until it receives a valid IP.
//
// Once the app receives a valid IP it will begin scrolling through the list
// of defined cities and display the status, humidity, current temp and
// high/low. It will also display the current city it is updating.
// After it has scrolled 10 times it will update again.
//
// Proxy configuration hardcoded in g_sDefaultParams.
//
// UART0, connected to the Virtual Serial Port and running at 115,200, 8-N-1,
// is used to display messages from this application.
//
//##### TEST_INSTRUCTIONS END #####
...
WeatherEvent(uint32_t ui32Event, void* pvData, uint32_t ui32Param)
{
    //##### INTERNAL BEGIN #####
#ifdef DEBUG
    UARTprintf("\nWeatherEvent() %08x, %08x, %08x", ui32Event, pvData,
            ui32Param);
#endif
    //##### INTERNAL END #####
...
EnetEvents(uint32_t ui32Event, void *pvData, uint32_t ui32Param)
{
    if(ui32Event == ETH_EVENT_CONNECT)
    {
        ...
        //##### INTERNAL BEGIN #####
#ifdef DEBUG_OUTPUT
        PrintIPAddress(g_pcIPAddr, EthClientServerAddrGet());
#endif
        //##### INTERNAL END #####
    }
